...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ([self.delegate respondsToSelector:@selector(joyStickCoordinateChangeDataX:WithY:)]) {
        x = x - backCenter.x;
        y = backCenter.y -y;
        [self.delegate joyStickCoordinateChangeDataX:x WithY:y];
        speedX = x;
        speedY = y;
    }
    
    timer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(timer_callback) userInfo:nil repeats:YES];
    
}
- (void)timer_callback
{
    [self.delegate joyStickCoordinateChangeDataX:speedX WithY:speedY];
}
...
//    NSLog(@" %f     %f  %f   %f", _M1, _M2, _straightSpeed, _turnSpeed);
...
//    if ([self.delegate respondsToSelector:@selector(joyStickCoordinateChangeDataX:WithY:)]) {
//        [self.delegate joyStickCoordinateChangeDataX:<#(CGFloat)#> WithY:<#(CGFloat)#>];
//    }
    //创建通知对象
